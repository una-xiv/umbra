        ...
        dataManager.GetExcelSheet<MainCommandCategory>()
            ...
            .ForEach(
                cmd => {
                    ...
                    _categories[(MenuCategory)cmd.RowId] = new((MenuCategory)cmd.RowId, cmd.Name.ToString().StripSoftHyphen());
                }
            );
                    ...
                    dataManager.GetExcelSheet<MainCommand>()
                        ...
                        .ForEach(
                            cmd => {
                                ...
                                MainMenuItem item = new(cmd.Name.ToString().StripSoftHyphen(), cmd.SortID, cmd.RowId) { Icon = icon };
    ...
    )
    {
        ...
        if (item != null) {
            ...
            if (entry is not null && !_player.HasItemInInventory(itemId)) {
                ...
                if (entry is null) {
                    category.AddItem(
                        new(item.Value.Name.ToString(), sortIndex, () => _player.UseInventoryItem(itemId)) {
