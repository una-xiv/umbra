...
/// <param name="iconId">The ID of the game icon to render.</param>
...
) : INode
{
    ...
    /// <summary>
    /// The ID of the game icon to render.
    /// </summary>
    ...
    private static readonly Dictionary<string, IDalamudTextureWrap> IconCache = [];
    private string CacheKey => $"{IconId}_{Grayscale}_{Rounding}";
    ...
    public void Render(ImDrawListPtr drawList, Rect rect, float elementOpacity)
    {
        ...
        if (IconCache.TryGetValue(crc, out var cachedIcon)) {
    ...
    private IDalamudTextureWrap GetTexture()
    {
        if (Rounding == 0 && Grayscale == 0) {
            return AssetManager.GetIcon(IconId);
        }

        var image = AssetManager.GetIconFile(IconId).GetImage();
        image.Mutate(ctx => {
