...
public class ThemeBrowserWindow : Window
{
    protected override string  UdtResourceName => "umbra.windows.theme.browser.xml";
    protected override string  Title           => I18N.Translate("Window.ThemeBrowser.Title");
    protected override Vector2 MinSize         => new(1000, 550);
    protected override Vector2 MaxSize         => new(1200, 900);
    protected override Vector2 DefaultSize     => new(1000, 600);
    ...
    private void UpdateThemes()
    {
        ...
        foreach (var theme in this._themes) {
            ...
            if (null == _selectedTheme) {
                SelectTheme(card, theme);
            }

            card.OnClick       += _ => SelectTheme(card, theme);
            card.OnDoubleClick += _ => {
                SelectTheme(card, theme);
                ApplySelectedTheme();
            };
        }
    ...
    private void SelectTheme(ThemeCardNode card, UmbraThemesApi.Theme theme)
    {
        foreach (var c in RootNode.QuerySelector("#theme-list")!.QuerySelectorAll(".theme-card")) {
            c.ToggleClass("selected", c == card);
        }

        ThemePreviewNode previewNode = new(theme);
        RootNode.QuerySelector("#preview-container")!.Clear();
        RootNode.QuerySelector("#preview-container")!.AppendChild(previewNode);
