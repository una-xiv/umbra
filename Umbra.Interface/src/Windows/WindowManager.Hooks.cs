...
{
    private unsafe delegate IntPtr AtkUnitBaseReceiveGlobalEvent(
        AtkUnitBase* thisPtr, ushort cmd, uint a3, IntPtr a4, uint* a5
    );
    ...
    private unsafe Hook<AtkUnitBaseReceiveGlobalEvent> CreateAtkUnitBaseReceiveGlobalEventHook(
    ...
    {
        switch (HonorEscapeKey) {
            case true when !_isHookEnabled:
                _isHookEnabled = true;
                _atkGlobalEventHook.Enable();
                break;
            case false when _isHookEnabled:
    ...
    /// </summary>
    private unsafe IntPtr AtkUnitBaseReceiveGlobalEventDetour(
        AtkUnitBase* thisPtr, ushort cmd, uint a3, IntPtr a4, uint* arg
    )
    {
        // 12 = SendKey, 3 = Close.
